...
using MassTransit;
using Sample.Contracts.Inventories;
using Warehouse.Contract;

namespace Sample.Components.StateMachines.Activities.Inventories
{
    public class AllocateInventoryActivity : IActivity<AllocateInventoryArguments, AllocateInventoryLog>
    {
        ...
        public async Task<ExecutionResult> Execute(ExecuteContext<AllocateInventoryArguments> context)
        {
            ...
            var response = await _client.GetResponse<InventoryAllocate>(new
            {
